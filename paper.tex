...
\begin{abstract}
This work describes a method to construct type-1 intuitionistic fuzzy
inference systems. This type of systems is able to handle more uncertainty than
a type-1 fuzzy inference system and performs faster than a type-2
fuzzy inference system. The concepts of intuitionistic membership, and
intuitionistic center of area are proposed, in order to implement a
system which is similar in design than the traditional fuzzy inference
systems. The proposed method was implemented and compared against type-1 fuzzy inference
...
optimize the parameters of each of the methods being compared. This
optimization ensures a fair comparison between each of the methods
against the proposed method. The results show that the intuitionistic
...
The use of intuitionistic fuzzy sets (IFSs) in a traditional fuzzy
inference system (FIS) should enable the handling of more
...
inferences in a type-2 FIS (T2-FIS) are very time consuming compared to a
type-1 FIS (T1-FIS). This is a consequence of the complex type
...
\cite{mendel2006interval}, which is faster than a general T2-FIS (GT2-FIS). A type-1
IFIS (T1-IFIS) should perform better than a T1-FIS in terms
...
uncertainty in mind, by L. A. Zadeh in 1965
\cite{zadeh1965fuzzy}. This uncertainty allows an
...
traditional logic. The same technique can be extended to use fuzzy
premises. In this case, all the
...
Mamdani FIS (FIS) \cite{mamdani1975experiment},
...
T2-FISs \cite{mendel2002type}, and those systems that used the
traditional fuzzy sets are now called type-1 fuzzy inference systems
(T1-FIS). T2-FISs have been proved to work better than T1-FIS in many
situations, often problems that involve high levels of uncertainty, such as
noisy data \cite{mendel2001uncertain}.

Another extension to fuzzy sets, called intuitionistic fuzzy sets, was
proposed by K. T. Atanassov \cite{atanassov1986intuitionistic}. This
extension adds the concepts of indeterminacy and non-membership to
traditional fuzzy sets. In traditional fuzzy sets, the non-membership
of an element is always the
...
somewhat tall, although I'm not very sure about my opinion'' to be
represented in a FIS.

The goal of this work is to propose a Mamdani IFIS that is
...
this way of representing an IFS (as in \cite{davarzani2013novel}). The
proposal in this paper is to use the
...
% Ya lo menciono en el método propuesto

% Lastly, in the proposed method, the intuitionistic membership
% functions (IMF) representing the consequents need to be
% aggregated. This aggregation can be done by applying the union
% operator over the different IFS. This operator is defined in
% \ref{atanassov2013intuitionistic} as in (\ref{ifs-union}).
...
necessary to construct a basic IFIS, as this system can only handle
Gaussian membership functions, and simple rules without AND or OR operators.
An implementation was completed in order to make comparisons against
traditional FISs, such as T1-FISs, IT2-FISs, and GT2-FIS, and prove that
the design was viable and able to contribute to better results than
the traditional FISs.
...
In many works, IFSs are constructed manually,
by interviewing experts who assign memberships and
non-memberships to each of the elements of a set (such as in %to each of the members of a set?
...
Figure \ref{fs-as-ifs}. This IFS represents a traditional fuzzy set, as it
...
The first step of the inference process in a traditional FIS (a flow chart
...
MFs. This is straightforward in a traditional FIS, as the
...
intuitionistic membership and is represented as $i\mu$. %(Mario)Creo que este podría ser
                                %el titulo
...
The next step in the inference process of a FIS is to perform an $\alpha$-cut % (Mario) La del FIS? aclaralo
on the associated consequents to the antecedents that are
activated by the inputs. In a traditional FIS, this process is straightforward, as
...
% Aclara que en los tipo Mamdani %% (A): ahhhhh cierto, cierto
fuzzy set in a Mamdani FIS, but the most frequently found in the literature is the
center of area (CoA) or centroid. One can obtain the centroid
of a traditional fuzzy set A by using (\ref{center-of-area}). For an
...
intuitionistic fuzzy sets instead of traditional fuzzy sets, i.e., that
...
where the proposed method is compared against T1-FISs and IT2-FISs
with uncertain mean and uncertain standard deviation.

The initial (and ambitious) hypothesis is that a T1-IFIS
...
that particular number of runs was because of trial and error. Initially,
...
creation of a T1-IFIS that can perform better than a traditional T1-FIS
and an IT2-FIS.
...
should inspire the creation of T2-IFIS (both the interval and general types).
