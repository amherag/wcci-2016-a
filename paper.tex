...
intuitionistic inference engine \cite{cornelis2001compositional} %Que diferencias hay?

implementation of a ifis, castillo \cite{castillo2007intuitionistic} %Por?  
...
This design was completed in order to make comparisons against
...
as is depicted in Fig. \ref{ifs}; and 2) setting different means and
standard deviations of the Gaussian membership and non-membership
functions, while keeping the $height$ of the MF equal
...
$i\mu$ (intuitionistic membership). %(Mario)Creo que este podría ser el titulo
...
The next step in the inference process is to perform an $\alpha$-cut % (Mario) La del FIS? aclaralo
...
it. There are a number of methods that can be used to defuzzify a %Aclara que en los tipo Mamdani
...
the performances of each of the FISs, and a Genetic Algorithm (GA) was
...
length from the individual with a 
...
The results of the experiment along with the code can be found on the 
following public Github repository: https://github.com/amherag/wcci-2016-a
